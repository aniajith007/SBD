    ...
    <div>
      <div className="bg-[#191A24] h-screen">
        <div className="flex flex-col items-center justify-center w-full h-full gap-8 sm:gap-16">
          ...
          {/* <div className='flex relative font-poppins text-xl text-white'>
            <TypeAnimation
              sequence={[
          ...
          {<div className='flex absolute font-poppins animate__animated animate__flash animate__infinite text-xl text-white'>
            {format(targetDate, "dd-MM-yyyy")}</div>}
          {days == 365 ? <Lottie
            animationData={anim}
            loop={true}
            style={{ width: 180, height: 180, display: "flex" }}
          /> : <Lottie
            animationData={anim2}
            loop={true}
...
function Countdown() {
  ...
  if (new Date().getMonth() == month - 1 && new Date().getDate() > day) {
    // For Same month and >24
    const date = new Date();
    const newDate = addYears(date, 1);
    console.log("from same month after bday Next Your Spl Day : ", newDate)
    year = newDate.getFullYear()
  }

  if (new Date().getMonth() > month - 1) {
    // For After feb month's
    const date = new Date();
    const newDate = addYears(date, 1);
    console.log("After bdaymonth  bday Next Your Spl Day : ", newDate)
    year = newDate.getFullYear()
  }
